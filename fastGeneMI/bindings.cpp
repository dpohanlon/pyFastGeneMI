...
py::array_t<double> mim_ML_py(const arma::Mat<int>& disc_expr_data, int n_cores) {
    arma::mat miMatrix = mim_ML_cpp(disc_expr_data, n_cores);
    return carma::mat_to_arr(miMatrix);
}

py::array_t<double> mim_MM_py(const arma::Mat<int>& disc_expr_data, int n_cores) {
    arma::mat miMatrix = mim_MM_cpp(disc_expr_data, n_cores);
    return carma::mat_to_arr(miMatrix);
}

py::array_t<double> mim_CS_py(const arma::Mat<int>& disc_expr_data, int n_cores) {
